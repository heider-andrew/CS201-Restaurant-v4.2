...
public class RestaurantPanel extends JPanel {
    ...
    private CookAgent cook = new CookAgent("W. Puck", restaurant, monitor);
    private MarketAgent market1 = new MarketAgent("Market 1", 1);
    private MarketAgent market2 = new MarketAgent("No Pizza hur!", 2);
    private MarketAgent market3 = new MarketAgent("Quikie Mart", 3);
    
    private Vector<CustomerAgent> customers = new Vector<CustomerAgent>();
    private Vector<WaiterInterface> waiters = new Vector<WaiterInterface>();
	...
	cook.myMarkets[0] = market1;
	cook.myMarkets[1] = market2;
	cook.myMarkets[2] = market3;
	...
	if(type.equals("Customers")){
		...
		CustomerInterface temp = customers.get(i);
		if(temp.getName() == name)
		    gui.updateInfoPanel(temp);
	    }
	}else if(type.equals("Waiters")){
	    for(int i=0; i < waiters.size(); i++){
		WaiterInterface temp = waiters.get(i);
		if(temp.getName() == name)
		    gui.updateInfoPanel(temp);
	    }
	}
	...
	if(type.equals("Customers")){
	    ...
	    if (name.equalsIgnoreCase("shared")){
	    	w = new SharedWaiter(name, aStarTraversal, restaurant, tables, monitor);
	    } else {
